...
def copy_images(dst_folder, src_images_paths, utms):
    ...
    for src_image_path, (utm_east, utm_north) in zip(
        tqdm(src_images_paths, desc=f"Copy to {dst_folder}", ncols=100), utms
    ):
        ...
        dst_image_name = util.get_dst_image_name(
            latitude,
            longitude,
            pano_id=src_image_name.split("_")[0],
            tile_num=tile_num,
            note=note,
        )
...
for dataset in ["train", "val", "test"]:
    matlab_struct_file_path = os.path.join(
        dataset_folder, "raw_data", "datasets", f"pitts30k_{dataset}.mat"
    )
    ...
    copy_images(
        os.path.join(dataset_folder, "images", dataset, "database"), g_images, g_utms
    )
    # Queries
    q_images = [os.path.join("queries_real", f"{f[0].item()}") for f in mat_struct[3]]
    q_utms = mat_struct[4].T
    copy_images(
        os.path.join(dataset_folder, "images", dataset, "queries"), q_images, q_utms
    )

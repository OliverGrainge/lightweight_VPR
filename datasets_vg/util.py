...
def get_distance(coords_A, coords_B):
    return math.sqrt(
        (float(coords_B[0]) - float(coords_A[0])) ** 2
        + (float(coords_B[1]) - float(coords_A[1])) ** 2
    )
...
def download_heavy_file(url, output_path):
    ...
    for attempt_num in range(10):  # In case of errors, try 10 times
        try:
            ...
            print(e)
            print(
                f"I'll try again to download {output_path} in {RETRY_SECONDS**attempt_num} seconds"
            )
            time.sleep(RETRY_SECONDS**attempt_num)
    else:
        raise RuntimeError(
            f"I tried 10 times and I couldn't download {output_path} from {url}"
        )
...
def is_valid_timestamp(timestamp):
    ...
    """
    return bool(
        re.match("^(\d{4}(\d{2}(\d{2}(_(\d{2})(\d{2})?(\d{2})?)?)?)?)?$", timestamp)
    )
...
def get_dst_image_name(
    ...
    (
        ...
        latitude,
        longitude,
    ) = format_location_info(latitude, longitude)
    ...
    assert is_valid_timestamp(
        timestamp
    ), f"{timestamp} is not in YYYYMMDD_hhmmss format"
    if roll is None:
        roll = ""
    else:
    ...
    return (
        f"@{easting}@{northing}@{zone_number:02d}@{zone_letter}@{latitude}@{longitude}"
...
class VideoReader:
    def __init__(self, video_name, size=None):
        ...
        self.video_length_in_millis = int(
            self.frames_num * 1000 / self.frames_per_second
        )
    ...
    def get_frame_at_frame_num(self, frame_num):
        ...
        if frame is None:
            return None  # In case of corrupt videos
    ...
    def __repr__(self):
        H, W = int(self.vc.get(cv2.CAP_PROP_FRAME_HEIGHT)), int(
            self.vc.get(cv2.CAP_PROP_FRAME_WIDTH)
        )
        return (
            f"Video '{self.video_name}' has {self.frames_num} frames, "

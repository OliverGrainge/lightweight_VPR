...
for src_image_path in tqdm(src_images_paths, ncols=100):
    (
        ...
        heading,
        pitch,
    ) = os.path.basename(src_image_path).split("_")
    pitch = pitch.replace(".jpg", "")
    dst_image_name = util.get_dst_image_name(
        latitude, longitude, pano_id, tile_num, heading, pitch, extension=".jpg"
    )
...
with open(
    join(raw_data_folder, "reference_poses_598", "reference_poses_addTM_all_598.txt"),
    "r",
) as file:
...
for line in lines:
    ...
    dst_image_name = util.get_dst_image_name(
        latitude, longitude, pano_id=image_id, heading=heading
    )
    dst_image_path = join(dst_queries_folder, dst_image_name)
    src_image_path = join(
        raw_data_folder,
        "BuildingQueryImagesCartoIDCorrected-Upright",
        f"{image_id}.jpg",
    )
    _ = shutil.move(src_image_path, dst_image_path)

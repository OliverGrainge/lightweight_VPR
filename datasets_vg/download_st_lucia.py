...
urls = [
    "https://mega.nz/file/nE4g0LzZ#c8eL_H3ZfXElqEukw38i32p5cjwusTuNJYYeEP1d5Pg",
...
for sequence_num, url in enumerate(urls):
    ...
    for frame_num, line in zip(tqdm(range(vr.frames_num)), lines):
        ...
        latitude = (
            "-" + latitude
        )  # Given latitude is positive, real latitude is negative (in Australia)
        ...
        if last_coordinates is None:
            ...
            distance_in_meters = util.get_distance(
                last_coordinates, (easting, northing)
            )
        ...
        image_name = util.get_dst_image_name(
            latitude, longitude, pano_id=f"{subset_name}_{frame_num:05d}"
        )

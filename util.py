        ...
        if "layer" in module_name:
            for basic_block_name, basic_block in module.named_children():
                torch.quantization.fuse_modules(
                    basic_block, [["conv1", "bn1"], ["conv2", "bn2"]], inplace=True
                )
        ...
        for i, module in enumerate(stage.modules()):
            if isinstance(module, models.shufflenetv2.InvertedResidual):
                for j, name in enumerate(["branch1.0", "branch1.1"]):
                    torch.quantization.fuse_modules(
                        module, [f"branch1.{j}.0", f"branch1.{j}.1"]
                    )
                for j, name in enumerate(
                    ["branch2.0", "branch2.1", "branch2.3", "branch2.4"]
                ):
                    torch.quantization.fuse_modules(
                        module, [f"branch2.{j}.0", f"branch2.{j}.1"]
                    )
    ...
    for name, module in model.named_children():
        if "conv" in name:
            if isinstance(module, torch.nn.Conv2d):
                ...
                torch.quantization.fuse_modules(
                    model, [name, name[: name.rfind(".")] + ".norm"], inplace=True
                )
...
def quantize_model(model, precision="fp16", calibration_dataset=None, args=None):
    if precision == "fp32":
        ...
        trt_model_fp32 = torch_tensorrt.compile(
            ...
            inputs=[
                torch_tensorrt.Input(
                    (args.infer_batch_size, 3, 480, 640), dtype=torch.float32
                )
            ],
        ...
        trt_mod = torch_tensorrt.compile(
            model,
            inputs=[torch_tensorrt.Input((args.infer_batch_size, 3, 480, 640))],
...
def benchmark_latency(
    ...
    if input is None:
        input_tensor = torch.randn(
            (batch_size,) + input_size, dtype=torch.float32
        ).cuda()
    else:

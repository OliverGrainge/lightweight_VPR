...
if (
    args.aggregation in ["netvlad", "crn"] and args.fc_output_dim is None
):  # If using NetVLAD layer, initialize it
    if not args.resume:
...
if (
    args.aggregation in ["netvlad", "crn"] and args.fc_output_dim is not None
):  # If using NetVLAD layer, initialize it
    if not args.resume:
        triplets_ds.is_inference = True
        if args.fc_output_dim != None:
...
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    recalls, _, _, recalls_str = test.test(args, val_ds, model)
...
recalls, _, _, recalls_str = test.test(
